...
RUN Rscript -e "library(dplyr)" -e "versions <-tibble::as_tibble(installed.packages())" -e "keras_version <-as.character( versions %>% filter(Package=='keras') %>% select(Version) %>% unlist() )"  -e "tensorflow_version <- as.character( versions %>% filter(Package=='tensorflow') %>% select(Version) %>% unlist() )"  -e "remotes::install_github('jcrodriguez1989/autokeras', keras = keras_version, tensorflow = tensroflow_version, upgrade='never',  clean = TRUE, Ncpus = 16)"
